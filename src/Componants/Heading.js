...
// const accordionSize=[{heading:'Education' , value:['10th', '12th', 'gradation', 'post-graduation'] }, {heading:"education",value:['10th', '12th', 'gradation', 'post-graduation']}]  
...
//   ]
  ...
  <>
       ...
       return <AccordionItem className='' key={index}>
         ...
         <AccordionPanel  className=''>  
          {item.value.map((values)=>{
            return <Checkbox   checked={isChecked} value={values} name={values}  onChange={(e)=>{onchange(item.heading, e.target.name)}} >{values}</Checkbox>
          })}
         </AccordionPanel>
         </AccordionItem>
              ...
              <button className='btn btn-outline-primary icons mt-3 ms-3'><FaPaperPlane/></button>
   ...
   <DrawerBody > 
     <div className='main-heading ' >
      ...
      <Accordion    mt={'5'} allowMultiple  className='accord'>
       ...
       return <AccordionItem className='' key={index}>
         ...
         <AccordionPanel  className=''>
        {item.value.map((values)=>{
             return <Checkbox onChange={(e)=>handleCheckboxChange(e,'nz')}  checked={isChecked} >{values}</Checkbox>
        })}
         </AccordionPanel>
         </AccordionItem>
    ...
    <DrawerFooter> 
      {/* <Button fontSize={'20'} width={'100px'} alignItems={'center'} border={'1px solid gray'} onClick={onClose}>Submit</Button>   */}
      <button className='btn btn-outline-primary'><FaPaperPlane/> submit</button>
       </DrawerFooter> 
     </DrawerContent> 
